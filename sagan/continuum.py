...
__all__ = ['WindowedPowerLaw1D', 'WindowedLinear', 'WindowedLinearCont', 'lrd_sphere', 'BlackBody', 'BalmerPseudoContinuum', 'extinction_ccm89']
...
class WindowedLinearCont(Fittable1DModel):
    ...
    def evaluate(self, x, c0, c1, x_0, x_min, x_max):
        y=c0+c1*(x - x_0)
        mask = (x >= x_min) & (x <= x_max)
        return np.where(mask, y, 0.0)
    
class lrd_sphere(Fittable1DModel):

    amp_norm= Parameter(default=1.0)
    ...
    def evaluate(self, x,  amp_norm, log_rou, log_Mbh):
        """Evaluate the lrd_sphere model at wavelength x."""
        interp_func=self.spec_interp_dict[f'1e{log_Mbh[0]:.0f}']
        model_spec=interp_func(np.array([[log_rou[0], wl] for wl in x]))  # input shape (N_points, 2), output \
        return amp_norm * model_spec                                      # spec in unit of erg/s/Angstrom
